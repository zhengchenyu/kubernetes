...
import (
	...
	cnatv1alpha1 "k8s.io/cnat/pkg/apis/cnat/v1alpha1"
	clientset "k8s.io/cnat/pkg/generated/clientset/versioned"
	cnatscheme "k8s.io/cnat/pkg/generated/clientset/versioned/scheme"
	informers "k8s.io/cnat/pkg/generated/informers/externalversions/cnat/v1alpha1"
	listers "k8s.io/cnat/pkg/generated/listers/cnat/v1alpha1"
)

const controllerAgentName = "cnat"
...
func NewController(
	...
	// Add cnat types to the default Kubernetes Scheme so Events can be
	// logged for cnat types.
...
func (c *Controller) syncHandler(key string) (time.Duration, error) {
	klog.Infof("Handle At, where key is %s", key)
	...
	if err != nil {
		if errors.IsNotFound(err) {
			utilruntime.HandleError(fmt.Errorf("at %s is not exist.", key))
			return time.Duration(0), nil
		}
		return time.Duration(0), err
	}

	klog.Infof("original is %s %s %s %s", original.Name, original.Namespace, original.Spec.Name, original.Status.Phase)
	...
	case cnatv1alpha1.PhasePending:
		klog.Infof("instace %s is PhasePending", key)
		klog.Infof("instance %s: checking schedule %q", key, instance.Spec.Scheduler)
		d, err := timeUntilSchedule(instance.Spec.Scheduler)
		if err != nil {
	...
	if !reflect.DeepEqual(original, instance) {
		...
		_, err = c.cnatclientset.CnatV1alpha1().Ats(instance.Namespace).Update(context.TODO(), instance,
			metav1.UpdateOptions{})
		if err != nil {
			klog.Infof("update state failed, caused by %s", err)
			return time.Duration(0), err
		}
	}
...
func timeUntilSchedule(schedule string) (time.Duration, error) {
	...
	if err != nil {
		klog.Infof("parse failed, schedule is %s , caused by", schedule, err)
		return time.Duration(0), nil
	}
...
func (c *Controller) updateFooStatus(foo *cnatv1alpha1.At, deployment *appsv1.Deployment) error {
	...
	_, err := c.cnatclientset.CnatV1alpha1().Ats(foo.Namespace).Update(context.TODO(), fooCopy, metav1.UpdateOptions{})
	return err
}
